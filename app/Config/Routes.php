...
$routes->group('peminjaman', $authFilter, function ($routes) use ($anggota, $admin) {
    // Tampilan Utama
    ...
    // Fitur Umum (Admin & Anggota)
    $routes->get('lihat_bukti/(:num)', 'Peminjaman::lihat_bukti/$1');

    // Fitur Khusus Admin / Petugas
    $routes->get('setujui_pembayaran/(:num)', 'Peminjaman::setujui_pembayaran/$1');
    $routes->match(['get', 'post'], 'konfirmasi/(:num)/(:any)', 'Peminjaman::konfirmasi/$1/$2');
    $routes->get('hilang/(:num)', 'Peminjaman::hilang/$1');
    $routes->get('hapus_riwayat/(:num)', 'Peminjaman::hapus_riwayat/$1');
    
    // --- PERBAIKAN DI SINI ---
    // Dipindahkan ke dalam group agar URL-nya menjadi: peminjaman/kirim_peringatan/ID
    $routes->get('kirim_peringatan/(:num)', 'Peminjaman::kirim_peringatan/$1');
});

// --- Management Dashboard Admin ---
$routes->group('admin', array_merge($authFilter, $admin), function ($routes) {
    $routes->get('dashboard', 'Admin\Dashboard::index');
});

// Route untuk anggota lihat inbox
$routes->get('inbox', 'Inbox::index', $authFilter);
// Route untuk halaman inbox anggota
$routes->get('inbox-anggota', 'InboxAnggota::index', ['filter' => 'auth']);
$routes->get('inbox-anggota/hapus/(:num)', 'InboxAnggota::hapus/$1', ['filter' => 'auth']);